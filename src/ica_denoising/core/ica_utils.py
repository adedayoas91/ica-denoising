...
def ica_dec(data, n_comps, t, max_, random_state=0):
    ica = FastICA(
...
def bss_dec(
    ...
    **kwargs,
    ...
    segment_lengths = kwargs.pop("segment_lengths", None)
    if method in {"fastica", "ica"}:
        return ica_dec(data, n_comps, t=t, max_=max_, random_state=random_state)
    if method == "infomax":
        return infomax_dec(
            data,
            n_comps=n_comps,
...
def sobi_dec(
    ...
    covs = _lagged_covariances(X_white, lags=lags, segment_lengths=segment_lengths)
...
def _lagged_covariances(X_white, lags, segment_lengths=None):
    covs = []
    n_samples = X_white.shape[0]
    segments = _segment_slices(n_samples, segment_lengths)
    for lag in lags:
        lag = int(lag)
        if lag <= 0 or lag >= n_samples:
            continue
        numerator = np.zeros((X_white.shape[1], X_white.shape[1]), dtype=float)
        n_pairs = 0
        for segment in segments:
            segment_data = X_white[segment]
            if segment_data.shape[0] <= lag:
                continue
            numerator += segment_data[lag:].T @ segment_data[:-lag]
            n_pairs += segment_data.shape[0] - lag
        if n_pairs == 0:
            continue
        cov = numerator / n_pairs
...
def _segment_slices(n_samples, segment_lengths=None):
    ...
    if sum(lengths) != n_samples:
        raise ValueError(
            f"segment_lengths sum to {sum(lengths)}, expected {n_samples} samples."
        )
    slices = []
    start = 0
