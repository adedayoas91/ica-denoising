...
from ica_denoising.behavior_decoding import BehaviorTargets, TraceVariant, blocked_folds
...
def fit_causal_state_model(
    ...
    validate_causal_state_config(config)
    ...
    for variant in variants:
        for shift in config.target_shifts:
            ...
            for fold_id, train_idx, test_idx in folds:
                ...
                x0_train = _apply_standardizer(x0_flat[train_idx], scaler)
                x0_test = _apply_standardizer(x0_flat[test_idx], scaler)
                ...
                transition = _make_transition_model(config)
                ...
                bout_threshold = float(
                    np.quantile(target_map["vigor"][train_idx], config.bout_quantile)
                )
                fold_bout = (target_map["vigor"] >= bout_threshold).astype(int)
                y_bout_train = fold_bout[train_idx]
                y_bout_test = fold_bout[test_idx]
                ...
                fold_rows.append(
                    {
                        ...
                        "held_out_latent_variance": held_out_latent_variance,
                        "dynamic_mse_normalized": dynamic_mse_normalized,
                        "persistence_mse": persistence_mse,
                        "mean_state_mse": mean_state_mse,
                        "dynamic_improvement_vs_persistence": persistence_improvement,
                ...
                for local_idx, sample_idx in enumerate(test_idx):
                    row = {
                        ...
                        "bout_state": int(fold_bout[sample_idx]),
                    }
                    for d in range(z0_test.shape[1]):
                        row[f"z{d + 1}"] = float(z0_test[local_idx, d])
...
def compare_latent_graphs(
    ...
    if "fold" in embeddings.columns:
        group_cols.append("fold")
    for group_key, group in embeddings.groupby(group_cols, dropna=False):
        variant = group_key[0]
        shift = group_key[1]
        fold = group_key[2] if len(group_key) > 2 else None
...
def minimum_causal_gap(config: CausalStateConfig) -> int:
    """Return the minimum sample gap that prevents neural-history overlap."""
    return config.window - 1


def validate_causal_state_config(config: CausalStateConfig) -> None:
    if config.window < 2:
        raise ValueError("config.window must be at least 2.")
    if config.latent_dim < 1:
        raise ValueError("config.latent_dim must be at least 1.")
    if not 0.0 <= config.bout_quantile <= 1.0:
        raise ValueError("config.bout_quantile must be between 0 and 1.")
    if config.gap < minimum_causal_gap(config):
        raise ValueError(
            f"config.gap={config.gap} is too small for window={config.window}; "
            f"use at least {minimum_causal_gap(config)} to prevent overlapping histories."
        )
    if config.transition_model not in {"linear", "mlp"}:
        raise ValueError("config.transition_model must be 'linear' or 'mlp'.")


def _make_transition_model(config: CausalStateConfig):
