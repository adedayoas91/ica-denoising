...
from behavior_decoding import BehaviorTargets, TraceVariant
...
class CausalBehaviorDecodingTests(unittest.TestCase):
    ...
    def test_fit_causal_state_model_outputs_tables(self) -> None:
        cfg = CausalStateConfig(window=10, target_shifts=(0, 1), latent_dim=3, n_splits=4, gap=2, random_state=1)
        ...
        for col in ("dynamic_mse", "angle_r2", "vigor_r2", "bout_balanced_accuracy"):
            self.assertIn(col, result.fold_metrics.columns)
        for col in ("variant", "time_index", "z1"):
    ...
    def test_markov_sufficiency_returns_fold_rows(self) -> None:
        ...
        z = x0_flat[:, :2]
        from behavior_decoding import blocked_folds
